...
let rats = [];
let ratTimer;  // id for rat timer

let humanX = 15;
let humanY = BOTTOM_ROW;
let runHuman = false;
let humanTimer;

let houseNum = 0;
let houses = [ "images/House0.bmp", "images/House1.bmp", "images/House2.bmp" ];
let housesIn = [ "images/House0i.bmp", "images/House1i.bmp", "images/House2i.bmp" ];
...
const myLoader = function ( imageData ) {

	// Blit the image to the grid at 0, 0

	PS.imageBlit( imageData, 0, 0 );
	if ( !finishedGame1 ) {
		bgColor = PS.color( 15, 31 ); // save background color of image for animation
	}
};

const tickRats = function () {
	// The length of the rats array is the current number of rats

	let len = rats.length; // number of rats
	let i = 0;
	while ( i < len ) {
		// get current position of rat

		let rat = rats[ i ];
		let x = rat[ 0 ];
		let y = rat[ 1 ];
		PS.color( x, y, bgColor ); // erase existing rat
		y -= 1;

		// If bead is above last row, erase it and redraw one bead lower

		if ( !finishedGame1 && ( y >= BOTTOM_ROW ) ) {
			PS.color( x, y, PS.COLOR_GRAY ); // redraw
			rat[ 1 ] = y; // save new y-pos
			i += 1;
		}
		else {
			rats.splice( i, 1 );
	...
	if ( finishedGame1 ) {
		PS.timerStop( ratTimer );
	}
};

const tickHuman = function () {
	if ( !runHuman ) {
		return;
	}

	// Erase human at current position

	PS.color( humanX, humanY, bgColor );
	humanY += 1;
	if ( humanY >= 32 ) {
		runHuman = false;
		PS.timerStop( humanTimer ); // done if reached bottom of screen
	}
	else {
		PS.color( humanX, humanY, 0xFFD8CF ); // redraw human at new position
	}
};
...
const loadGame1 = function () {
	...
	houseNum = 0;
	PS.imageLoad( houses[ houseNum ], myLoader );
	PS.statusText( "CLICK below house to Infest." );
	waiting = false;
	change_music( music_home );
	ratTimer = PS.timerStart( 6, tickRats );
};
	...
	if ( sequenceUser[ indexOfArray ] === sequenceMade[ indexOfArray ] ) {
		...
		if ( sequenceMade.length === sequenceUser.length ) {
			...
			if ( level == fluteGoal ) {
				...
				nextSequence();
				PS.gridFade( 15, { rgb : PS.COLOR_GREEN } );
		...
		PS.gridFade( 30, { rgb : PS.COLOR_RED } );
		PS.gridColor( PS.COLOR_WHITE );
...
const wonRats = function () {
	...
	PS.statusText( "Success! You removed all the rats!" );
	PS.gridFade( 15, { rgb : PS.COLOR_GREEN } );
...
const _tick = function () {
	if ( !_running || _dead || finishedGame3 ) {
		return;
	}
...
const loadGame3 = function () {
	...
	PS.imageLoad( "images/houseBg.bmp", myLoader );
...
PS.init = function ( system, options ) {
	...
	PS.audioLoad( "FluteSong", {
		...
		loop : true,
		onLoad : function ( data ) {
			music_flute = data.channel; // save the channel ID
		}
	} );
	...
	PS.audioLoad( "mute", {
		...
		loop : true,
		onLoad : function ( data ) {
			mute = data.channel; // save the channel ID
		}
	} );
...
PS.touch = function ( x, y, data, options ) {
	...
	if ( game1 && !waiting && !finishedGame1 ) {
		...
		if ( totalRats >= ratGoal ) {
			totalRats = 0;
			humanY = BOTTOM_ROW;
			humanTimer = PS.timerStart( 6, tickHuman );
			runHuman = true;
			PS.audioPlay( "fx_wilhelm" );
			houseNum += 1;
			if ( houseNum >= houses.length) {
				finishedGame1 = true;
				PS.statusText( "Infestations complete! SPACE to continue." );
			}
			else {
				ratGoal += 1;
				PS.statusText( "House infested! SPACE to continue." );
				waiting = true;
			}
		}
	}
...
PS.keyUp = function ( key, shift, ctrl, options ) {
	...
	if ( game1 && ( key === 32 ) ) {
		if ( finishedGame1 ) {
			...
			waiting = false;
			PS.imageLoad( houses[ houseNum ], myLoader );
			PS.statusText( "Infest another house!" );
		}
	}
